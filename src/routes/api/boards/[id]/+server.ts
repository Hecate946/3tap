...
function etagFor(updatedAt: string | null | undefined) { return updatedAt ? `\"${updatedAt}\"` : null; }
...
export async function GET(event: RequestEvent) {
  ...
  const board = await getBoard(id, metadata, since);
  return json(board, { headers: { ...(etag ? { etag } : {}), 'cache-control': 'private, no-store' } });
}
