...
function cleanChange(value: unknown): Change | null {
  ...
  if (
    ...
    !validDate(item.date) ||
    ![0, 1, 2].includes(item.value as number)
  ) return null;
...
export async function PUT(event: RequestEvent) {
  ...
  const habitIds = [...new Set(changes.map((change) => change.habitId))];
  ...
  const writes = changes.filter((change) => change.value !== 0);
  const deletes = changes.filter((change) => change.value === 0);
    ...
    const results = await Promise.all(
      deletes.map((change) =>
        db
          ...
          .delete()
          .eq('board_id', id)
          .eq('habit_id', change.habitId)
          .eq('entry_date', change.date)
      )
    );
    const failed = results.find((result) => result.error);
    if (failed?.error) throw error(500, failed.error.message);
  }
